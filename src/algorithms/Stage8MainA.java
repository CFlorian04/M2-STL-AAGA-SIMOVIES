...
public class Stage8MainA extends Brain {
    //---PARAMETERS---//
    ...
    private static final int MOVEFROMENEMY = 4;
    ...
    public void step() {
        ...
        if(state == MOVEFROMENEMY) {
            double distance = Math.sqrt((targetY - myY) * (targetY - myY) + (targetX - myX) * (targetX - myX));
            if(distance >= Parameters.bulletRange) {
                if(isSameDirection(myGetHeading(), getAngle(targetX, targetY))) {
                    myMove();
                }
                else {
                    stepTurn(Parameters.Direction.RIGHT);
    ...
    private void firePosition(double x, double y){
        ...
        if(distance >= Parameters.bulletFiringLatency) { return ;}
    ...
    private double getAngle(double targetX, double targetY) {
        double angle = (double) Math.toDegrees(Math.atan2(targetY - myY, targetX - myX));
